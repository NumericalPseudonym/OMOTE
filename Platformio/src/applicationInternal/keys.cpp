...
void doShortPress(char keyChar, int keyCode)
{
  ...
  if ((currentMillis - lastTimeSent[keyCode / keypadROWS][keyCode % keypadROWS]) > repeatRate)
  {
    ...
    {
      omote_log_d("doShortPress: key '%c', will use command '%u'.\r\n", keyChar, command);
      executeCommand(command);
    }
    else
    {
      omote_log_w("doShortPress: key '%c', but no command defined.\r\n", keyChar);
    }
  }
  else
  {
    omote_log_v("doShortPress: key '%c' too recently pressed.\r\n", keyChar);
  }
}
...
void doLongPress(char keyChar, int keyCode)
{
  ...
  if (command != COMMAND_UNKNOWN)
  {
    omote_log_d("doLongPress: key '%c' (long press), will use command '%u'.\r\n", keyChar, command);
    executeCommand(command);
  }
  else
  {
    omote_log_w("doLongPress: key '%c' (long press), but no command defined.\r\n", keyChar);
  }
}
...
void keypad_setKeyStatesAndCheckForHold()
{
  ...
  for (uint8_t row = 0; row < keypadROWS; row++)
  {
    for (uint8_t col = 0; col < keypadCOLS; col++)
    {
      ...
      if ((rawKeys[row][col].rawKeyState == PRESSED_RAW) 
          && (currentMillis - rawKeys[row][col].timestampReceived) > KEY_HOLD_TIME)
      {
...
void keypad_processKeyStates()
{
  ...
  for (uint8_t row = 0; row < keypadROWS; row++)
  {
    for (uint8_t col = 0; col < keypadCOLS; col++)
    {
      ...
      {
        //omote_log_v("keypad_processKeyStates:***************************************************\r\n");
        omote_log_v("keypad_processKeyStates: Key '%c' pressed.\r\n", keyChar);
        // omote_log_v("keypad_processKeyStates: Key help '%s', keyState '%d'.\r\n",  keyStateProcessed[row][col].keyIsHold ? "true" : "false", keyStateProcessed[row][col].keyState);

        repeatModes keypressMode = get_key_repeatMode(gui_memoryOptimizer_getActiveSceneName(), keyChar);
      
        if ((keypressMode == SHORT)
            && (keyStateProcessed[row][col].keyState != PRESSED))
        {
          omote_log_v("keypad_processKeyStates: PRESSED of SHORT key '%c' '%d'.\r\n", keyChar, keyCode);
          doShortPress(keyChar, keyCode);
        }
        else if ((keypressMode == SHORT_REPEATED) 
                 && (keyStateProcessed[row][col].keyState != PRESSED))
        { 
          // here do not repeat it too early, do the repeat only in HOLD
          omote_log_v("keypad_processKeyStates: PRESSED of SHORT_REPEATED key '%c' '%d'.\r\n", keyChar, keyCode);
          doShortPress(keyChar, keyCode);
        }
        else
        {
          omote_log_v("keypad_processKeyStates: not SHORT or SHORT_REPEATED '%d'.\r\n", keypressMode);
        }
        keyStateProcessed[row][col].keyState = PRESSED;
      }
      else if (singleKeyState == HOLD)
      {
        omote_log_v("keypad_processKeyStates: Key '%c' held.\r\n", keyChar);
        repeatModes keypressMode = get_key_repeatMode(gui_memoryOptimizer_getActiveSceneName(), keyChar);
        
        if ((keypressMode == SHORTorLONG) 
            && (keyStateProcessed[row][col].keyState != HOLD))
        {
          omote_log_v("keypad_processKeyStates: HOLD of SHORTorLONG key %c (%d)\r\n", keyChar, keyCode);
        ...
        { 
          // this is the only case where we do not check the keyStateProcessed, because here it is intended to repeat the action
          omote_log_v("keypad_processKeyStates: HOLD of SHORT_REPEATED key %c (%d)\r\n", keyChar, keyCode);
          doShortPress(keyChar, keyCode);
        }
        else
        {
          omote_log_v("keypad_processKeyStates: not HOLD or SHORTorLONG or SHORT_REPEATED.\r\n");
        }
        keyStateProcessed[row][col].keyState = HOLD;
      }
      else if (singleKeyState == RELEASED)
      {
        omote_log_v("keypad_processKeyStates: Key '%c' released.\r\n", keyChar);

        if ((get_key_repeatMode(gui_memoryOptimizer_getActiveSceneName(), keyChar) == SHORTorLONG) 
            && !keyStateProcessed[row][col].keyIsHold && (keyStateProcessed[row][col].keyState != RELEASED))
        {
          omote_log_v("keypad_processKeyStates: value of keyIsHold for keycode %d is %d\r\n", keyCode, keyStateProcessed[row][col].keyIsHold);
          omote_log_v("key: RELEASED of SHORTorLONG key '%c' '%d'.\r\n", keyChar, keyCode);
          doShortPress(keyChar, keyCode);
        }
        omote_log_v("keypad_processKeyStates: will set keyIsHold to FALSE for keycode '%d'.\r\n", keyCode);
        keyStateProcessed[row][col].keyIsHold = false;

        omote_log_v("keypad_processKeyStates: press of key '%c' '%d'.\r\n", keyChar, keyCode);
