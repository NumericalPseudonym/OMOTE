...
void executeCommandWithData(uint16_t command, commandData commandData, std::string additionalPayload = "")
{
  ...
  case MQTT:
  {
    ...
    omote_log_d("executeCommandWithData: will send MQTT, topic '%s', payload '%s'.\r\n", topic.c_str(), payload.c_str());
  ...
  case BLE_KEYBOARD:
  {
    omote_log_d("executeCommandWithData: will send BLE keyboard command '%u', payload '%s', additionalPayload '%s'.\r\n", command, convertStringListToString(commandData.commandPayloads).c_str(), additionalPayload.c_str());
  ...
  case SCENE:
  {
    ...
    omote_log_d("executeCommandWithData: will send scene command to the sceneHandler.\r\n");
  ...
  case GUI:
  {
    ...
    omote_log_d("executeCommandWithData: will send gui command to the sceneHandler.\r\n");
  ...
  case SPECIAL:
  {
    if (command == MY_SPECIAL_COMMAND)
    {
      ...
      omote_log_d("executeCommandWithData: could execute a special command here, if you define one.\r\n");
    }
    break;
  }
  }
...
void executeCommand(uint16_t command, std::string additionalPayload)
{
  try
  {
    ...
    {
      omote_log_d("executeCommand: will execute command '%u' with additionalPayload '%s'.\r\n", command, additionalPayload.c_str());
      executeCommandWithData(command, commands.at(command), additionalPayload);
    }
    else
    {
      omote_log_w("executeCommand: command '%u' not found.\r\n", command);
    }
  }
